...
from jarvis.enterprise.audit import AuditLogger
from jarvis.perception.wake_word import WakeWord
...
def test_wake_word_strips_keyword_from_command():
    cfg = Config(); cfg.require_wake_word = True
    w = WakeWord(cfg)
    heard, command = w.strip("jarvis what time is it")
    assert heard is True
    assert "jarvis" not in command.lower()
...
def test_audit_chain_is_tamper_evident():
    ...
    bus.publish(Event.TRANSCRIPT, {"text": "jarvis hello", "confidence": 1.0})
