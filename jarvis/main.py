"""JARVIS entrypoint — constructs and wires every layer, then runs.

    python -m jarvis        (from the repo root)
or  python run.py
...
def banner(config: Config, registry: SkillRegistry, stt: SttEngine,
           audio: AudioInput, tts: TtsEngine, backend: str) -> None:
    print("=" * 58)
    print("  J.A.R.V.I.S  —  voice-driven AI operating layer")
    print("=" * 58)
    print(f"  AI engine     : {'Claude API' if config.has_ai else 'local (rule-based)'}")
    ...
    print(f"  wake word     : {'REQUIRED — say “Jarvis …”' if config.require_wake_word else 'off'}")
    ...
    print("-" * 58)
    print("  Say “Jarvis” then your command, or type in DEV INPUT + Enter.")
...
def main() -> None:
    ...
    banner(config, registry, stt, audio, tts, backend)

    diagnostic = {
        "ai_engine": "claude" if config.has_ai else "local",
...
def _run_web(config, bus, orchestrator, boot_subsystems, shutdown) -> None:
    from .visual.web_window import JarvisWebWindow

    window = JarvisWebWindow(config, bus, orchestrator)
...
def _run_qt(config, bus, orchestrator, boot_subsystems, shutdown) -> None:
    ...
    from .visual.qt_window import JarvisQtWindow
    ...
    window = JarvisQtWindow(config, bus, orchestrator)
    window.set_on_close(shutdown)
    window.show()
    window.start()
    boot_subsystems()
...
def _run_tk(config, bus, orchestrator, boot_subsystems, shutdown) -> None:
    ...
    from .visual.main_window import JarvisWindow

    root = tk.Tk()
    window = JarvisWindow(root, config, bus, orchestrator)
    window.set_on_close(shutdown)
    boot_subsystems()
    window.start()
    try:
