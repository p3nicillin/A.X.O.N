...
rings. Every visual property is driven by :class:`JarvisState` plus live audio
...
_PALETTE = {
    JarvisState.IDLE:      ((40, 170, 255), 0.010, 0.9),
    JarvisState.LISTENING: ((60, 210, 255), 0.020, 1.15),
    JarvisState.THINKING:  ((150, 90, 255), 0.045, 1.0),
    JarvisState.SPEAKING:  ((90, 230, 255), 0.025, 1.2),
    JarvisState.ERROR:     ((255, 70, 70), 0.030, 0.85),
}
...
class CoreRenderer:
    ...
    def step(self) -> None:
        ...
        self.yaw += spin * (2 if self.state == JarvisState.THINKING else 1)
        self.phase += 0.05
        self.audio *= 0.85      # decay live levels
        self.speak *= 0.80

        flicker = (random.uniform(0.7, 1.0)
                   if self.state == JarvisState.THINKING else 1.0)
        jitter = (lambda: random.uniform(-3, 3)) if self.state == JarvisState.ERROR \
            else (lambda: 0.0)
    ...
    def _draw_waveform(self, rgb) -> None:
        ...
        active = self.state in (JarvisState.LISTENING, JarvisState.SPEAKING)
        if not active:
            self.cv.itemconfig(self._wave_id, fill="")
            return
        amp = (self.speak if self.state == JarvisState.SPEAKING else self.audio)
