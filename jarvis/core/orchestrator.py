...
It owns the state machine and is the ONLY component that sets JarvisState or
...
from .states import JarvisState
...
class Orchestrator:
    def __init__(self, config: Config, bus: EventBus, registry: SkillRegistry,
        ...
        self.ai = build_engine(config, registry.catalogue())
        ...
        engine_name = type(self.ai).__name__
        self._log("info", f"AI engine: {engine_name}")
    ...
    def _on_transcript(self, msg) -> None:
        ...
        # The wake spotter already detected "Jarvis" and captured the command,
    ...
    # the system stays testable without saying "Jarvis" every time.
    ...
    def _respond(self, text: str, error: bool = False) -> None:
        self.set_state(JarvisState.ERROR if error else JarvisState.SPEAKING)
        if self.audio_input is not None:
