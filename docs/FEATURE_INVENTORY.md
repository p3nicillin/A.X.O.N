...
| `BrowserAutomationSkill` | Thread-affine isolated Playwright Chromium; public URL guard on every request; stable visible-element IDs plus confirmation-gated click/fill | Structured text, links, element bounds, and before/after verification render in AXON; lifecycle shutdown closes owned browser processes |
| `WorkflowControlSkill` | Atomic per-step checkpoints; list/resume/cancel commands; critic and confirmation gates are reapplied on resume; private inputs are redacted | Recoverable workflow count and history are exposed in Diagnostics; interrupted safe plans survive restart |
| `NativeAutomationSkill` | Process-isolated Microsoft UI Automation with hard timeouts plus Win32 fallback; stable per-snapshot IDs; protected credential blocking; confirmation-gated click/fill | Semantic roles, supported patterns, labels and bounds render in AXON; mutations include before/after verification and expected-outcome checks |
