...
| `NativeAutomationSkill` | Active-window-only Win32 control enumeration; stable per-snapshot IDs; abort-if-hung messages; confirmation-gated click/fill | Control roles, labels and bounds render in AXON; mutations include before/after verification and expected-outcome checks |
