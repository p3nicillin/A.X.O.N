...
chain so it genuinely *radiates* light, brightening as AXON speaks.

Pipeline (all offscreen FBOs, then composited to the widget's framebuffer)::

    scene  -> sceneFBO      (full res, procedural AXON core, HDR-ish)
...
_PALETTE = {
    AxonState.IDLE:      ((0.16, 0.66, 1.00), (0.47, 0.86, 1.00), 0.25, 0.0),
    AxonState.LISTENING: ((0.24, 0.82, 1.00), (0.67, 0.94, 1.00), 0.60, 1.0),
    AxonState.THINKING:  ((0.59, 0.43, 1.00), (1.00, 0.67, 0.94), 0.95, 1.0),
    AxonState.SPEAKING:  ((0.35, 0.90, 1.00), (0.71, 1.00, 0.90), 0.70, 0.0),
    AxonState.ERROR:     ((1.00, 0.27, 0.27), (1.00, 0.71, 0.35), 0.85, 0.0),
}
...
# --- the AXON core itself, drawn procedurally -----------------------------
_SCENE = """
#version 330 core
in vec2 v_uv;
out vec4 frag;
