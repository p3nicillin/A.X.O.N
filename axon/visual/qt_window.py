"""The PySide6 AXON window: the GLSL core as a glowing backdrop, with a
...
class HudOverlay(QWidget):
    ...
    def _title(self, p) -> None:
        self._text(p, 34, 30, "A.X.O.N", _ACCENT, 26, bold=True)
        self._text(p, 36, 58, "JUST  A  RATHER  VERY  INTELLIGENT  SYSTEM",
                   _DIM, 8, bold=True)

    def _status(self, p) -> None:
        label, col = {
            AxonState.IDLE:      ("◉ ONLINE", _OK),
            AxonState.LISTENING: ("◉ LISTENING", _ACCENT_HI),
            AxonState.THINKING:  ("◉ PROCESSING", _ACCENT_HI),
            AxonState.SPEAKING:  ("◉ SPEAKING", _OK),
            AxonState.ERROR:     ("⚠ ALERT", _ERR),
        }[self.state]
    ...
    def _indicators(self, p, w) -> None:
        rows = [("LISTENING", AxonState.LISTENING),
                ("PROCESSING", AxonState.THINKING),
                ("SPEAKING", AxonState.SPEAKING)]
        ry = 92
...
class AxonQtWindow(QMainWindow):
